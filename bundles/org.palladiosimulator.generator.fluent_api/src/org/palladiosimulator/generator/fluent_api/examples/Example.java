package org.palladiosimulator.generator.fluent_api.examples;

import org.palladiosimulator.generator.fluent_api.component.factory.FluentRepositoryFactory;
...
class Example {
    ...
    public static void readmeExampleBackend() {
        // Factory
        final RepositoryFactory repoFact = RepositoryFactory.eINSTANCE;
        // Repository
        final Repository repository = repoFact.createRepository();

        // Database component
        final BasicComponent databaseComponent = repoFact.createBasicComponent();
        databaseComponent.setEntityName("Database");

        // IDatabase interface
        final OperationInterface databaseInterface = repoFact.createOperationInterface();
        databaseInterface.setEntityName("IDatabase");

        // Signature store
        final OperationSignature store = repoFact.createOperationSignature();
        store.setEntityName("store");
        // with parameters forename, name
        final Parameter forename = repoFact.createParameter();
        forename.setParameterName("forename");
        forename.setDataType__Parameter(null); // referencing the imported data types poses another
                                               // problem
        final Parameter name = repoFact.createParameter();
        ...
        final OperationProvidedRole dbProvIDb = repoFact.createOperationProvidedRole();
        ...
        final OperationInterface webInterface = repoFact.createOperationInterface();
        databaseInterface.setEntityName("IWeb");

        final OperationSignature submit = repoFact.createOperationSignature();
        submit.setEntityName("submit");
        // with parameters forename, name
        final Parameter forename2 = repoFact.createParameter();
        forename2.setParameterName("forename");
        forename2.setDataType__Parameter(null);
        final Parameter name2 = repoFact.createParameter();
        name2.setParameterName("forename");
        name2.setDataType__Parameter(null);

        final OperationProvidedRole webProvIweb = repoFact.createOperationProvidedRole();
